...
# סגנון קבוע, דריסת כיווניות לתרשים וריווח אפשרויות הרדיו בתפריט השאלות
st.markdown("""
<style>
...
# חלוקת העמודות הראשית של כל המסך (צד ימין לפילטרים ותפריט, צד שמאל לגרפים)
menu_col, chart_col = st.columns([1.1, 2.7], gap="small")

with menu_col:
    # אזור הכותרת והפילטרים בצד ימין
    ...
    # תפריט בחירת שאלה ממוקם כעת גם הוא בצד ימין, מתחת לפילטרים
...
with chart_col:
        ...
        if labels:
            ...
            def add_points(source_filter, source_name):
                ...
                for i, ans in enumerate(labels):
                    ...
                    if s_val is not None and m_val is not None:
                        ...
                        s_val = round(s_val, 1); m_val = round(m_val, 1)
                        if s_val == m_val:
                            txt_pos.append("top center" if source_filter == "שילוב" else "bottom center")
            ...
            for trace in fig.data:
                if trace.y:
                    ...
                    if valid_y: all_plotted_values.extend(valid_y)
    ...
    # ==============================================================================
    # --- כרטיס תרשים נתח שוק (SOV) מוערם 100% ---
    # ==============================================================================
    ...
    channel_colors = {
        ...
        "ערוץ קשת 12": "#FAA046", 
        "ערוץ רשת 13": "#82BE64", 
        "ערוץ עכשיו 14": "#DC5050", 
        "ערוץ i24news (אפיק 15)": "#5A96D2" 
    }
