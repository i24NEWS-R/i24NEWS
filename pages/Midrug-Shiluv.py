...
menu_col, chart_col = st.columns([1, 4], gap="small")
...
with menu_col:
    ...
    with st.container(border=True):
        ...
        st.info("כאן תוכלו לבחור איזה נתונים לראות: באיזה ימי מחקר להתמקד (אמצע שבוע או סופי שבוע), איזה סקר להציג מבין הגלים שנערכו (לכל מחקר יש 2 גלים שמוצגים בנפרד או כממוצע) ובאיזה אוכלוסייה להתמקד (אפשר לבחור פירוט דמגורפי רק כאשר בוחרים להציג ממוצע של שני גלים)")
...
with chart_col:
    ...
    with st.container(border=True):
        if card1_labels:
            #########################################
            # כרטיס ראשון - סקר מול מדרוג: כותרת ותרשים
            #########################################
    ...
    st.warning("בחלק הזה ניתן לראות את תוצאות הסקר מול תוצאות המדרוג. בטבלה ניתן לראות את הפערים בין המדרוג לסקר - שינוי אבסולוטי הוא הנתון של המדרוג פחות הנתון של הסקר. שינוי מחושב לוקח בחשבון את הפער הקבוע של הסקר מהמדרוג")
        
        #########################################
        # כרטיס שני - נתח שוק: מסגרת
        #########################################
        with st.container(border=True):
            #########################################
